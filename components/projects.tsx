...
function TagBadge({ label }: { label: string }) {
  return (
    <span className="rounded-md border border-white/[0.08] bg-white/[0.04] px-2.5 py-0.5 text-xs text-white/50">
      {label}
    </span>
  )
}
    ...
    >
      ...
      <h3
        className={`text-lg font-medium ${
          isInProgress ? "text-white/30" : "text-white/90"
        }`}
      >
        {project.title}
      </h3>
      ...
      {project.github && !isInProgress && (
        <div className="mt-auto pt-2">
          <a
            ...
            className="inline-flex items-center gap-1.5 text-xs font-medium text-white/40 transition-colors duration-150 hover:text-indigo-400"
          >
...
export function Projects() {
  return (
    <section id="projects" className="mx-auto max-w-4xl px-6 py-24 bg-[#030303]">
      <h2 className="mb-12 text-2xl font-semibold text-white/90">
        <span className="mr-2 bg-gradient-to-r from-indigo-400 to-rose-400 bg-clip-text text-transparent">/</span>Projects
      </h2>
